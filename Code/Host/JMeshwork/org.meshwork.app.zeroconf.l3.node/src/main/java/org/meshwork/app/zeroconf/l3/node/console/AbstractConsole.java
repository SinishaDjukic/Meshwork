...
public abstract class AbstractConsole {
    ...
    protected void printUsage() {
        System.out.println("Usage: <zero config> <serial config> <serial device>");
        System.out.println("Example: device-001.cfg serial.cfg /dev/ttyUSB0");
        System.out.println();
    }
    ...
    protected void initApp(String[] args) {
        try {
            this.args = args;
            ZeroConfiguration routerConfig = initRouterConfiguration(args[0].trim());
    ...
    protected void initApp(ZeroConfiguration routerConfig, SerialConfiguration serialConfig, AbstractMessageTransport transport, PrintStream ps) throws Exception {
        ...
        ZeroConfNode zeroConfDevice = initRouter();
        zeroConfDevice.init(transport, dispatcher, routerConfig, writer);
    }
...
//        return new IndentedPrintWriter(out, true, " ");
    ...
    protected ZeroConfNode initRouter() {
        return new ZeroConfNode();
    }

    protected abstract MessageDispatcherImpl initMessageDispatcher(MessageAdapter adapter, AbstractMessageTransport transport, ZeroConfiguration routerConfig, PrintWriter writer) throws Exception;
    ...
    protected ZeroConfiguration initRouterConfiguration(String config) throws Exception {
        ZeroConfiguration result = null;
        InputStream is = null;
        try {
            result = new ZeroConfiguration();
