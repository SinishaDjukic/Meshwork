...
#ifndef MW_LOG_ZEROCONFPERSISTENT
	#define MW_LOG_ZEROCONFPERSISTENT		MW_FULL_DEBUG
#endif
...
class ZeroConfPersistent: public ZeroConfSerial::ZeroConfListener {
	...
	public:
		ZeroConfPersistent(EEPROM* eeprom, zctype_configuration_t* configuration, uint16_t eepromStartOffset):
			m_eeprom(eeprom),
		...
		void read_configuration() {
			...
			MW_LOG_DEBUG(MW_LOG_ZEROCONFPERSISTENT, "Full configuration read. Start=%d, Count=%d", ((uint8_t*) (m_eepromStartOffset + ZC_STRUCT_DATA_START)), (size_t) sizeof(zctype_configuration_t));
		}
		...
		void devconfig_updated() {
			...
			MW_LOG_DEBUG(MW_LOG_ZEROCONFPERSISTENT, "Device config updated", NULL);
		}
		...
		void serial_updated() {
			...
			MW_LOG_DEBUG(MW_LOG_ZEROCONFPERSISTENT, "Serial updated", NULL);
		}
		...
		void network_updated() {
			...
			MW_LOG_DEBUG(MW_LOG_ZEROCONFPERSISTENT, "Network updated", NULL);
		}
		...
		void reporting_updated() {
			...
			MW_LOG_DEBUG(MW_LOG_ZEROCONFPERSISTENT, "Reporting updated", NULL);
		}
		...
		void factory_reset() {
			...
			MW_LOG_DEBUG(MW_LOG_ZEROCONFPERSISTENT, "Factory reset. EEPROM flashed to: %d", ZC_INIT_EEPROM_MEM_VALUE);
		}
};
#endif
