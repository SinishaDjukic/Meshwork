...
int MeshV1::sendWithACK(uint8_t attempts, uint16_t attemptsDelay,
	...
	IFMESHV1LOG TRACE_LOG("attempts=%d, attemptsDelay=%d, ack=%d, ackTimeout=%l", attempts, attemptsDelay, ack, ackTimeout);
	IFMESHV1LOG TRACE_LOG("dest=%d, port=%d, msg=%d, bufACK=%d, maxACKLen=%d", dest, port, msg, bufACK, maxACKLen);
#ifdef SUPPORT_DELIVERY_ROUTED
	IFMESHV1LOG TRACE_LOG("returnRoute=%d, returnRouteSize=%d", returnRoute, returnRouteSize);
#endif
	...
	IFMESHV1LOG TRACE_VP_BYTES(PSTR("L2 DATA SEND: "), toSend);

	for (int i = 0; i < attempts; i ++) {
		if ((result = m_driver->send(dest, port, vp)) < 1) {
			IFMESHV1LOG ERR("Driver send failed: %d", result);
			break;
		}
		//we wait for ACK if not broadcast
		if (ack != 0) { //need nwk ack
			IFMESHV1LOG DEBUG("Wait ACK", NULL);
			...
			do {
				...
				IFMESHV1LOG TRACE_LOG("Reply byte count=%d", reply_len);
				
				if ( reply_result > 0 ) {
					get_msg(&reply_msg, dataACK, reply_result);
					IFMESHV1LOG TRACE_LOG("Reply port=%d, Port=%d, Reply seq=%d, Seq=%d", reply_port, port, reply_msg.nwk_ctrl.seq, seq);
					IFMESHV1LOG TRACE_LOG("Deliv is ACK=%d", (reply_msg.nwk_ctrl.delivery & ACK), reply_port, port);
...
//								|| reply_port != port )// not sure if this is always the case?
							...
							IFMESHV1LOG DEBUG("Message not ACK, ignoring, code: %d", reply_result);
							IFMESHV1LOG DEBUG("\t\tDetails: port=%d, seq=%d, deliv=%d, reply src=%d", reply_port, reply_msg.nwk_ctrl.seq, reply_msg.nwk_ctrl.delivery, reply_src);
						}
						//TODO check reply_result-wrong.txt
				}
				//TODO should check if the error is caused by lenACK overflow
				IFMESHV1LOG TRACE_LOG("Reply result code=%d", reply_result);
			} while (reply_result < 0 && (RTC::since(start) < ackTimeout));
			IFMESHV1LOG TRACE_LOG("Out of loop w code: %d, reply len: %d", reply_result, reply_len);
			
			if ( reply_result >= 0 && reply_len > 0 )
				IFMESHV1LOG TRACE_ARRAY(PSTR("L2 DATA RECV: "), dataACK, reply_len);
			...
			if (reply_result >= 0) {
				if (reply_result == MeshV1::OK_MESSAGE_IGNORED) {
					...
					if ( returnRoute != NULL && (msg->nwk_ctrl.delivery & DELIVERY_ROUTED
						...
						IFMESHV1LOG TRACE_LOG("Copying route, hopcCount=%d", hopCount);
...
#endif
	if ( result >= 1 ) {
		IFMESHV1LOG TRACE_LOG("Result: %d:", result);
	} else {
		IFMESHV1LOG ERR("Send failed, code: %d", result);
	}
	return result;
}

int MeshV1::sendDirectACK(Mesh::Network::ACKProvider* ackProvider, univmsg_t* msg, uint8_t hopSrc, uint8_t hopPort) {
	IFMESHV1LOG TRACE_LOG("ackProvider=%d, msg=%d, hopSrc=%d, hopPort=%d", ackProvider, msg, hopSrc, hopPort);
	...
	if (ackProvider != NULL) {
		...
		if (bufACKsize > 0) {
			IFMESHV1LOG DEBUG("Send ACK with payload size: %d", bufACKsize);
	...
	IFMESHV1LOG DEBUG("Send ACK to node: %d via node: %d", origin, dest);
	iovec_t toSend[MAX_IOVEC_MSG_SIZE];
	iovec_t* vp = toSend;
	vp = get_iovec_msg_direct(vp, &reply_msg);
	
	IFMESHV1LOG TRACE_VP_BYTES(PSTR("L2 DATA SEND DIRECT ACK: "), toSend);
	
	if (m_driver->send(dest, hopPort, vp) < 1) {//send back ACK
		result = MeshV1::ERROR_ACK_SEND_FAILED;
		IFMESHV1LOG ERR("Driver send failed, code: %d", result);
	} else {
		result = MeshV1::OK;
	}
	IFMESHV1LOG TRACE_LOG("Result: %d", result);
...
#ifdef SUPPORT_DELIVERY_ROUTED
int MeshV1::sendRoutedACK(Mesh::Network::ACKProvider* ackProvider, univmsg_t* msg, uint8_t hopSrc, uint8_t hopPort) {
	IFMESHV1LOG TRACE_LOG("ackProvider=%d, msg=%d, hopSrc=%d, hopPort=%d", ackProvider, msg, hopSrc, hopPort);
	...
	IFMESHV1LOG TRACE_VP_BYTES(PSTR("L2 DATA SEND ROUTED ACK: "), toSend);
...
int MeshV1::send(uint8_t delivery, uint8_t retry,
	...
	IFMESHV1LOG TRACE_LOG("delivery=%d, retry=%d, dest=%d, port=%d, buf=%d, len=%d, bufACK=%d, lenACK=%d", delivery, retry, dest, port, buf, len, bufACK, lenACK);
	...
	if (retry >= -1) {
		if (len <= PAYLOAD_MAX) {
			...
			if (deliv & DELIVERY_DIRECT) {
				IFMESHV1LOG DEBUG("Send DIRECT", NULL);
			...
			if (result <= 0 && (deliv & DELIVERY_ROUTED)) {
				IFMESHV1LOG DEBUG("Send ROUTED", NULL);
						...
						for ( int i = 0; i < routeCount; i ++ ) {
							...
							if (route != NULL) {
								if (route->hopCount > m_maxHops) {
									IFMESHV1LOG NOTICE("Max hops reached, ignoring", NULL);
									continue;
	//								return (MeshV1::ERROR_MESSAGE_IGNORED_MAX_HOPS_REACHED);
						...
						IFMESHV1LOG NOTICE("No routes known to: %d", dest);
					}
				}
			}
#ifdef SUPPORT_DELIVERY_FLOOD
			if (result <= 0 && (deliv & DELIVERY_FLOOD)) {
				IFMESHV1LOG DEBUG("Send FLOOD", NULL);
				...
				if ( result > 0 ) {
					IFMESHV1LOG DEBUG("Route discovery success, hopCount=%d", hopCount);
...
*/
					...
					IFMESHV1LOG NOTICE("No routes found  to: %d", dest);
				}
			}
#endif
#endif
...
int MeshV1::recv(uint8_t& srcA, uint8_t& portA,
	...
	IFMESHV1LOG TRACE_LOG("src=%d, port=%d, newData=%d, newDataLenMax=%d, ms=%l, ackProvider=%d", srcA, portA, newData, newDataLenMax, ms, ackProvider);
	...
	if (result > 0) { //not timeouted, no crc error
		IFMESHV1LOG TRACE_LOG("Raw data count: %d", result);
		IFMESHV1LOG TRACE_ARRAY(PSTR("L2 DATA RECV: "), data, result);
//		trace.print(data, result, IOStream::hex, MeshV1::PAYLOAD_MAX);
		...
		if ( (data[0] & ACK) ) {//explicit ACKs are ignored, as they are handled within send
			IFMESHV1LOG NOTICE("Ignoring explicit ACK", NULL);
			...
			if ( !m_driver->is_broadcast() ) {//send to a specific destination
				if (recv_msg.nwk_ctrl.delivery == DELIVERY_DIRECT) { //Direct Send
					IFMESHV1LOG DEBUG("Recv DIRECT", NULL);
					...
					IFMESHV1LOG DEBUG("Payload len: %d", len);
					...
					if (devaddr == recv_msg.msg_routed.route_info.route.dst) { //we are the route dest
						IFMESHV1LOG DEBUG("Recv DIRECT to us", NULL);
						...
						IFMESHV1LOG DEBUG("Recv DIRECT to reroute", NULL);
						...
						if (myHop > 0) {//offset by 1 since we use it for bitmask
							if ( !(recv_msg.msg_routed.route_info.breadcrumbs & (1 << (myHop - 1))) ) {//our bit not set
								...
								IFMESHV1LOG TRACE_ARRAY(PSTR("L2 DATA SEND REROUTE: "), data, dataLen);
			...
			//TODO check why this fails at the receiver?!
				IFMESHV1LOG DEBUG("Recv BROADCAST delivery=%d", recv_msg.nwk_ctrl.delivery);
				if (recv_msg.nwk_ctrl.delivery == DELIVERY_DIRECT) {
					IFMESHV1LOG DEBUG("Recv BROADCAST DIRECT, will not ACK", NULL);
					...
					IFMESHV1LOG DEBUG("Recv BROADCAST FLOOD from addr=%d, hopCount=%d", devaddr, routeHops);
					if (devaddr == recv_msg.msg_flood.flood_info.route.dst) {//we are the ultimate receiver, ask for payload and generate a routed ACK
						...
						IFMESHV1LOG DEBUG("Message to us, sending ROUTED ACK", NULL);
						...
						if (myHop == 0) {//not in the hop list, add us
							IFMESHV1LOG NOTICE("Will rebroadcast", NULL);
							...
							IFMESHV1LOG TRACE_ARRAY(PSTR("L2 DATA SEND REBROADCAST: "), newMsg, dataLen);
							
							result = m_driver->send(Wireless::Driver::BROADCAST, port, newMsg, dataLen);//don't care about errors?
							if ( result < 1 )
								IFMESHV1LOG NOTICE("Driver send failed, code: ", result);
						} else {//we're in the hop list meaning we already retransmitted the message, so ignore
							IFMESHV1LOG NOTICE("Already rebroadcasted, ignoring", NULL);
						}
						result = MeshV1::OK_MESSAGE_IGNORED;
#endif
					} else {
